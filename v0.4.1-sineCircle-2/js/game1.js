...
function update2d() {
	...
	for (i=downpour.length-1; i >= 0; i--){
		speed += 0.001;
		downpour[i].x = canvas2d.width/2 + Math.sin(-speed+ (i*0.06))*180 - downpour[i].width/2 ;
  		downpour[i].y = canvas2d.height/2 + Math.cos(-speed+ (i*0.06))*180 - downpour[i].height/2 ;
		downpour[i].draw();
	}
}
