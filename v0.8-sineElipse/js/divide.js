...
function update2d() {
	...
	for (i=downpour.length-1; i >= 0; i--){
		speed += 0.07;
		downpour[i].x = canvas2d.width/3 + (Math.sin(speed*i*0.01)*90) - downpour[i].width/2 ;
  		downpour[i].y = canvas2d.height/3 + (Math.cos(speed*i*0.01)*90)/3 - downpour[i].height/2 ;
		downpour[i].draw();
	}
	//magic1 = Math.random()*0.18;
	//magic2 = Math.random()*0.4;
}
